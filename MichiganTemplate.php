...
/**
 * QuickTemplate class for Michigan skin
 * @ingroup Skins
 */
class MichiganTemplate extends BaseTemplate {
	/* Functions */
	...
	private function getTiles( $messageName = 'michigan-tiles' ) {
		/**
	...
	private function getAvatar( $width ) {
		...
		if ( $this->config->get( 'MichiganDisableAvatar' ) ) {
	...
	public function execute() {
		...
		if ( $this->config->get( 'MichiganUseIconWatch' ) ) {
		...
		if ( $user->isLoggedIn() ) {
			...
			$userNameTop = $skin->msg( 'michigan-guest' )->text();
		}
		...
		<div id="mw-head">
			...
			<?php
			if ( $this->config->get( 'MichiganSiteName' ) ) {
				?>
							...
							<h4 class="title-name">
								<a href="<?php echo $this->data['nav_urls']['mainpage']['href']; ?>">
									<span class="title-name">
										<?php
										if ( $this->config->get( 'MichiganSiteNameLogo' ) ) {
											?>
										...
										<?php
											...
											if ( $this->config->get( 'MichiganSiteNameText' ) ) {
												echo $GLOBALS['wgMichiganSiteText'];
											} else {
			...
			?>
			<?php
			if ( $this->config->get( 'MichiganLine' ) ) {
				?>
				<?php
				if ( $this->config->get( 'MichiganSiteName' ) ) {
					?>
			...
			?>
			<?php
			if ( $this->config->get( 'MichiganDownArrow' ) ) {
				?>
				<?php
				if ( $this->config->get( 'MichiganSiteName' ) ) {
					?>
			...
			<div id="left-navigation">
				<?php
				if ( $this->config->get( 'MichiganUploadButton' ) && $user->isAllowed( 'upload' ) ) {
					if (
			...
			<div id="right-navigation">
				...
				<?php
				if ( $this->config->get( 'MichiganSearchBar' ) ) {
					$this->renderNavigation( [ 'SEARCH' ] );
				}
				?>
				<div class="michiganMenu" id="usermenu">
					<div class="no-js">
					...
					<div class="topleft">
						<div class="tilebar" id="bartile">
							<div id="tilegrouptable">
								<div id="tilegroup">
									<?php
									if ( $this->config->get( 'MichiganBartile' ) ) {
										echo $this->getTiles();
									} else {
										echo $this->getTiles( 'michigan-tiles-second' );
									}
									?>
								</div>
							</div>
		...
		<div id="mw-panel-right" class="noprint">
			...
			$skin = $this;
			Hooks::run( 'MichiganRightPanel', [ &$skin ] );
			?>
		</div>
...
<?php
		...
		foreach ( $portals as $name => $content ) {
			...
			switch ( $name ) {
				...
				case 'TOOLBOX':
					...
					// Hook point for the ShoutWiki Ads extension
					$skin = $this;
					Hooks::run( 'MichiganAfterToolbox', [ &$skin ] );
					break;
				case 'LANGUAGES':
	...
	protected function renderNavigation( $elements ) {
		...
		foreach ( $elements as $name => $element ) {
			switch ( $element ) {
				case 'NAMESPACES':
					?>
					<div id="p-namespaces" role="navigation" class="michiganTabs<?php
				...
				case 'VARIANTS':
					?>
					<div id="p-variants" role="navigation" class="michiganMenu<?php
				...
				case 'VIEWS':
					?>
					<div id="p-views" role="navigation" class="michiganTabs<?php
				...
				case 'ACTIONS':
					?>
					<div id="p-cactions" role="navigation" class="michiganMenu actionmenu<?php
					if ( count( $this->data['action_urls'] ) == 0 ) {
						echo ' emptyPortlet';
					} 
					?>" aria-labelledby="p-cactions-label">
						<h5 id="p-cactions-label"><span><!-- this spot intentionally blank --></span></h5>
				...
				case 'SEARCH':
					...
					<div id="p-search" role="search">
						...
						<form action="<?php $this->text( 'wgScript' ) ?>" id="searchform">
							<div
								<?php echo $this->config->get( 'MichiganUseSimpleSearch' ) ? ' id="simpleSearch"' : '' ?>
							>
							<?php
	...
	public function makeLink( $key, $item, $options = [] ) {
		...
		if ( isset( $options['michigan-wrap'] ) && $options['michigan-wrap'] ) {
	...
	public function makeListItem( $key, $item, $options = [] ) {
		...
		if (
			$this->config->get( 'MichiganUseIconWatch' )
		...
		if (
			isset( $options['michigan-collapsible'] ) && $options['michigan-collapsible'] ) {
