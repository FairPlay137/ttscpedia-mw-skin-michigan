/**
 * Collapsible navigation for Michigan
 */
( function ( mw, $ ) {
	'use strict';
	var map;
	...
	function toggle( $element ) {
		...
		$.cookie(
			'michigan-nav-' + $element.parent().attr( 'id' ),
			isCollapsed,
			{ expires: 30, path: '/' }
		);
	...
	$( function ( $ ) {
		...
		$( '#mw-panel > .portal:not(.persistent)' )
			.each( function ( i ) {
				...
				// Re-save cookie
				if ( state !== null ) {
					$.cookie( 'michigan-nav-' + $( this ).attr( 'id' ), state, { expires: 30, path: '/' } );
				}
			} );
